class SuffixTree(object):
    ...
    def lss(self, node):
        if len(node.out) == 0:
            return ""
        childSubs = [self.lss(node.out[key]) for key in node.out if key!='#']
        childSubs.sort(key=lambda s: len(s), reverse=True)
        lsub = (node.lab if node.lab is not None else "") + childSubs[0]
        return lsub
    #       
    def longestSharedSubstring(self):
        """ return the longest shared substring"""
        self.lsub = ""
        lsub = self.lss(self.root)
        return lsub
